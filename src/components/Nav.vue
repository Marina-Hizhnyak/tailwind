...
<template>
  <nav class="bg-gray-900 fixed top-0 w-full z-50">
    <div class="container mx-auto flex items-center justify-between p-4">
      ...
      <button class="text-white block md:hidden focus:outline-none" type="button" @click="isMenuOpen = !isMenuOpen"
      ...
      <div :class="{ 'hidden md:flex': !isMenuOpen }" class="w-full md:w-auto md:flex-grow">
        ...
        <form class="flex items-center mt-4 md:mt-0">
          <input class="form-input rounded-lg border border-gray-300 py-1 px-2 focus:ring focus:border-blue-300"
            type="search" placeholder="Recherche" aria-label="Search" />
          <button class="ml-2 bg-green-500 text-white px-4 py-1 rounded-lg hover:bg-green-600" type="submit">
            Recherche
          </button>
        </form>
      </div>
